...
ROI_MIN_AREA           = 0.02   # ROI < 전체의 N% → 노이즈, 풀 프레임 사용
...
class MultiCamDiffMonitor:
    ...
    def __init__(self, n_cams,
        ...
        self.before_frames = [None] * n_cams
...
def main():
    ...
    parser.add_argument("--quorum",        type=int,   default=2,
                        help="fusion quorum (기본 2)")
    parser.add_argument("--min_corroborate", type=int, default=2,
                        help="occlusion weight 제외 기준 (기본 2)")
    parser.add_argument("--timed_log", default=None,
                        help="CSV: (time_sec, class_name, action) 이벤트 로그")
    ...
    print(f"처리 시작... diff_thresh={args.diff_thresh}  settle={args.settle}프레임\n")
    ...
    expected_calls = max(frame_idx // 2, 1)
    print(f"\n완료: {frame_idx}프레임 / YOLO 호출 {yolo_calls}회 "
          f"(skip=2 기준 {expected_calls}회 대비 {yolo_calls/expected_calls*100:.1f}%)")
