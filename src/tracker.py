...
class Sort:
    ...
    def _do_match(self, detections: List[Dict], predicted: List,
        ...
        """매칭 수행. Returns (matched_det_set, matched_trk_set)."""
    ...
    def update(self, detections: List[Dict]) -> List[Dict]:
        ...
        matched_det, _ = self._do_match(detections, predicted)
...
class ByteSort(Sort):
    """
    ByteTrack-inspired 2-stage matching.
    Stage 1: conf >= high_thresh → 모든 track
    Stage 2: conf < high_thresh → Stage 1 미매칭 track만 (새 track 생성 안 함)
    """
...
class MultiCameraTracker:
    ...
    Args:
        ...
        max_age      : 미감지 허용 최대 프레임 수
        min_hits     : 확정까지 필요한 연속 감지 횟수
        iou_threshold: 탐지-track 매칭 최소 IoU
        tracker_type : "sort" (기본) | "bytetrack"
        high_thresh  : ByteSort 전용, stage 분기 confidence 기준 (기본 0.6)
    """
