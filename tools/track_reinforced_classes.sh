#!/bin/bash
# 이번 재학습(2차, SAM2 3단계문턱)에서 집중 보강한 클래스들의 에폭별 AP/F1 추이를
# output/reinforced_classes_progress.txt에 스냅샷으로 남기고 git에 push
...
OUT="output/reinforced_classes_progress.txt"

{
    echo "=== $(date '+%Y-%m-%d %H:%M:%S') snapshot (source: $LOG) ==="
...
git add "$OUT"
git commit -m "reinforced classes progress snapshot $(date '+%Y-%m-%d %H:%M:%S')"
git push origin feat/rfdetr-sam2

echo "완료: $OUT에 스냅샷 추가하고 push함"
